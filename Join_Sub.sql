...
-- Alias (별칭) : 쿼리문에서 사용되는 별칭, 컬럼 및 테이블에서 사용가능, 결과 혹은 원래 이름을 다르게 지정하고 싶을떄 사용 ( AS 키워드 사용)
SELECT employee_number AS '사번',
	   name AS '사원 이름', 
       age AS '사원 나이', 
...
-- INNER JOIN : 두 테이블에서 조건이 일치하는 레코드 반환
-- SELECT column, ... FROM 기준테이블 INNER JOIN 조합할테이블 ON 조인조건
SELECT 
	E.employee_number '사원 번호', E.name '사원 이름', E.age'사원 나이', D.department_code'부서 코드', D.name'부서 코드', D.tel_number'부서 전화번호'
FROM employee E 
...
-- 만약 조합할 테이블에 조건에 부합하는 레코드가 존재하지 않으면 null 채움
SELECT 
...
-- CROSS JOIN 결과 레코드 수 = 기존 테이블 레코드 수 * 조합할 테이블의 레코드 수
...
SELECT E.employee_number '사번', E.name '사원 이름', D.name '부서명'
FROM employee E INNER JOIN department D
ON E.department_code = D.department_code
WHERE E.department_code = 'A';

-- 부서명이 '영업부'인 사원에 대해 사번, 이름, 나이를 조회하시오.
SELECT E.employee_number '사번', E.name '사원 이름', E.age '나이'
FROM employee E RIGHT JOIN department D
ON E.department_code = D.department_code
WHERE D.name = '영업부';


-- 서브쿼리 (Subquery) : 쿼리 내부에 존재하는 또 다른 쿼리, 쿼리 결과를 조건이나 테이블로 사용할 수 있도록 함
-- WHERE 절에서 서브쿼리 : 조회 결과를 조건으로 사용하여 동적으로 지정할 수 있도록 함
-- 주의! WHERE 절에서 비교 연산등으로 사용할 때 서브쿼리의 결과 컬럼 수 및 레코드 수를 주의
SELECT employee_number, name, age 
FROM employee
WHERE department_code = (SELECT department_code FROM department WHERE name = '영업부');

-- Error : 1241 WHERE 조건절에서 서브쿼리를 사용할땐 일반적으로 해당 서브쿼리의 결과 컬럼은 1개가 와야함
SELECT employee_number, name, age 
FROM employee
WHERE department_code = (SELECT * FROM department WHERE name = '영업부');

-- Error : 1242 WHERE 조건절에서 서브쿼리를 사용할땐 연산자에 따라 레코드의 개수를 잘 확인해야함
SELECT employee_number, name, age 
FROM employee
...
-- Error : 1241 컬럼은 반드는 하나만 와야함
SELECT employee_number, name, age 
FROM employee
WHERE department_code = (SELECT * FROM department);
...
ON E.department_code = D.department_code;
-- 서브쿼리를 FROM 절에서 사용할 떈 3개 이상의 테이블을 조인해서 결과를 얻고자 할떄 아주 유용하게 사용됨
