...
-- 데이터의 무결성을 보장, 잘못된 데이터 처리가 발생했을 때 롤백을 가능하게 해줌
...
-- Durability (지속성) : 트랜잭션이 성공적으로 완료되면 데이터는 영구히 저장되어야함

-- START TRANSACTION : 트랜잭션의 시작을 알림
-- COMMIT : 트랜잭션의 변경 사항을 데이터베이스에 영구히 반영 (트랜잭션의 성공적 완료를 뜻함)
-- ROLLBACK : 트랜잭션의 작업 내용을 이전 상태로 복원
-- SAVEPOINT : 트랜잭션 내부에서 롤백시 돌아올 위치 지정
