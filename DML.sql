...
-- 데이터 조작어 (DML)
-- 테이블에 데이터를 삽입, 조회, 수정, 삭제 할때 사용하는 SQL문
...
-- 지정한 컬럼의 순서와 데이터의 순서가 일치해야 함, 만약에 default값이 없으며 not null인 컬럼은 반드시 데이터 삽입해야 함
...
-- 3. 레코드 선택 조회
-- SELECT 조회할레코드, ... FROM 테이블명 WHERE 조건;
SELECT * FROM example_table WHERE column1 IS NULL;

-- UPDATE : 테이블에서 레코드를 수정할 때 사용하는 명령어
-- UPDATE 테이블명 SET 컬럼명 = 변경할데이터, ... WHERE 조건
UPDATE example_table SET column1 = 'B'; -- 해당컬럼이 전부 변경
UPDATE example_table SET column1 = 'C' WHERE example_column2 = 'B'; -- example_column2가 B인 레코드의 column1의 값을 수정

-- DELETE : 테이블에서 레코드를 삭제할 때 사용하는 명령어
-- DELETE FROM 테이블명 WHERE 조건;
DELETE FROM example_table WHERE column1 = 'C'; -- column1 의 C의 값이 들어가있는 레코드라인 삭제
DELETE FROM example_table; -- 모든 레코드 삭제 
